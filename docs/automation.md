...
3. The `sync-wiki.yml` workflow runs only on `main` documentation/Skill changes or from **Actions → Run workflow** after that secret exists. Before the one-time secret setup, the sync job is skipped rather than failing. It copies only files in `.searchcans-generated-pages` and never recursively wipes unknown Wiki pages.
